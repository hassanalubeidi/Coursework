...
int old_to_new_money(int pounds, int shillings, int pennies) {
...
void printSumPSP( int xPounds,
	...
	int x_in_pennies = old_to_new_money(xPounds, xShillings, xPennies);
	int y_in_pennies = old_to_new_money(yPounds, yShillings, yPennies);

	int total_pennies = x_in_pennies + y_in_pennies;

	struct OldMoney old_money_total = new_to_old_money(total_pennies);
	printf("Adding %d pounds, %d shillings, and %d pennies,\n", xPounds, xShillings, xPennies);
	printf("and %d pounds, %d shillings, and %d pennies,\n", yPounds, yShillings, yPennies);
	printf("gives %d pounds, %d shillings and %d pennies\n\n", old_money_total.pounds, old_money_total.shillings, old_money_total.pennies);
