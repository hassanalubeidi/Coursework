...
int main(int argc, char **argv) {
  ...
  int no_of_arguments = (int)argc;
  if(no_of_arguments > 2 && no_of_arguments < 5) { // 3 or 4 args
    struct play *list = read_plays(argv[1]); // read list based on arg name
    
    if(no_of_arguments == 4) {
      struct play *filtered_list = filter_user(atoi(argv[3]), list);
      list = filtered_list;
    }
    if(strcmp(argv[2], "c") == 0) { // strcomp saved me
      printf("%d\n", count_plays(list));
    } else if(strcmp(argv[2], "p") == 0) {
      print_plays(list);
    }
  } else printf("wrong number of arguments given");
  exit_usage();
}
